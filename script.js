...
document.addEventListener("DOMContentLoaded", function () { fetch("Danh sach sinh vien.xlsx") .then(res => { if (!res.ok) throw new Error("Không tìm thấy file Excel"); return res.arrayBuffer(); }) .then(data => { const workbook = XLSX.read(data, { type: "array" }); const sheet = workbook.Sheets[workbook.SheetNames[0]]; const json = XLSX.utils.sheet_to_json(sheet); hienThi(json); }) .catch(err => { console.error("Lỗi load file:", err); }); });

// ================= LOAD FILE EXCEL =================
const input = document.getElementById("fileInput"); if (input) { input.addEventListener("change", handleFile); }
